...
// FnotimeStore application context.
type FnotimeStore struct {
...
func (d *FnotimeStore) sendToBackend(w http.ResponseWriter, method, route, contentType, remoteAddr string, body *bytes.Reader) {
...
func (d *FnotimeStore) proxyStatus(w http.ResponseWriter, r *http.Request) {
...
func (d *FnotimeStore) proxyTimestamp(w http.ResponseWriter, r *http.Request) {
	b, err := ioutil.ReadAll(r.Body)
	r.Body.Close()
	if err != nil {
		util.RespondWithError(w, http.StatusBadRequest,
...
func (d *FnotimeStore) proxyVerify(w http.ResponseWriter, r *http.Request) {
	b, err := ioutil.ReadAll(r.Body)
	r.Body.Close()
	if err != nil {
		util.RespondWithError(w, http.StatusBadRequest,
...
func (d *FnotimeStore) status(w http.ResponseWriter, r *http.Request) {
	defer r.Body.Close()
...
func (d *FnotimeStore) timestamp(w http.ResponseWriter, r *http.Request) {
	defer r.Body.Close()
...
func (d *FnotimeStore) verify(w http.ResponseWriter, r *http.Request) {
	defer r.Body.Close()
...
func _main() error {
	...
	if !fileExists(loadedCfg.HTTPSKey) &&
		...
		err := util.GenCertPair("fnotimed", loadedCfg.HTTPSCert,
			loadedCfg.HTTPSKey)
		if err != nil {
